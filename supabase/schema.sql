-- ================================================================
-- GPA Tracker Schema v2.1  【 首次运行 / 需要重置结构时运行 】
...
-- ----------------------------------------------------------------
--
-- ⚠ 迁移：如果你的 learning_sessions.form 仍是 smallint，请在 Supabase SQL Editor 执行：
--   alter table public.learning_sessions alter column form type varchar(50)
--     using case form
--       when 1 then '自主复习' when 2 then '自主练习' when 3 then '自主预习'
--       when 4 then '校外线上' when 5 then '校外线下' when 6 then '课外作业'
--       when 7 then '学校作业' when 8 then '学校课堂' else '自定义'
--     end;
...
create table public.learning_sessions (
  ...
  category smallint not null,          -- 1=学习 2=复习 3=练习
  form     varchar(50) not null,        -- 自主复习 / 自主练习 / 自主预习 / 校外线上 / 校外线下 / 课外作业 / 学校作业 / 学校课堂 / <自定义>
  eval_type smallint not null default 1, -- 1=主观 2=客观

  self_rating smallint check (self_rating in (20,40,60,80,100)), -- 主观 5 档（可空，客观评估时不使用）
  grade_label varchar(5),                -- 客观：A+,A,A-,B+,B,B-,C+,C,C-,D+,D,D-,F（可空，主观评估时不使用）
  score smallint check (score between 0 and 100),

  -- ⚠ 迁移：如果实际数据库中 self_rating 或 grade_label 有 NOT NULL 约束，请执行：
...
-- 信号计算函数：在 (student, course, chapter) 粒度写最多 3 个信号
--   hesitant  ——  同一章节 ≥ 3 次记录且 self_rating 有 ≤50 / score 多次 <60
--   slow      ——  同一章节 ≥ 4 次记录且 score 始终 <70
--   growth    ——  同一章节最近 4 次记录里，score 的增量 ≥ +10
...
begin
  -- 先清理这个 (student, course) 下所有过期或重建的信号
  ...
  loop
    ...
    -- 指标：自评 ≤50 的次数；客观分 <60 的次数；最近 4 条评分 <70 是否全中
...
-- 触发器：每次插入或更新学习记录时，为 (student, course) 重新计算信号
...
-- learning_sessions：学生写自己、读自己；导师读所有
drop policy if exists sessions_select on public.learning_sessions;
create policy sessions_select on public.learning_sessions
  for select using (student_id = auth.uid() or public.is_mentor());
